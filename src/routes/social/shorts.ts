...
shortsRoute.post("/", requireAuth, async (c) => {
    ...
    const fileExt = file.type.split("/")[1] === "quicktime" ? "mov" : file.type.split("/")[1] || "mp4";
...
shortsRoute.post("/feed", collectAuth, async (c) => {
    ...
    try {
        ...
        const algoScore = sql<number>`((70.0 * ${apv}) + (30.0 * ${likeRate})) / POWER(${ageInHours} + 1.0, 1.5)`.as("algo_score");
        ...
        const algoQuery = database
            .select({
                id: shorts.id, title: shorts.title, videoUrl: shorts.videoUrl,
                creator: users.username, creatorDisplayName: users.displayName, creatorAvatarUrl: users.avatarUrl,
                views: shorts.views, likesCount: shorts.likesCount, watchDuration: shorts.watchDuration,
                videoLength: shorts.videoLength, createdAt: shorts.createdAt, score: algoScore
            })
            .from(shorts)
        ...
        const discoveryQuery = database
            .select({
                ...
                creator: users.username, creatorDisplayName: users.displayName, creatorAvatarUrl: users.avatarUrl,
                views: shorts.views, likesCount: shorts.likesCount, watchDuration: shorts.watchDuration,
                videoLength: shorts.videoLength, createdAt: shorts.createdAt, score: sql<number>`0.0`.as("score")
            })
            .from(shorts)
            .innerJoin(users, eq(shorts.userId, users.userId))
        ...
        while (aIndex < algoShorts.length || dIndex < discoveryShorts.length) {
            for (let i = 0; i < 4 && aIndex < algoShorts.length; i++) combinedFeed.push(algoShorts[aIndex++]);
...
shortsRoute.post("/:id/watch", async (c) => {
    ...
    let body;
    try { body = await c.req.json(); } catch { return c.json({ success: false }, 400); }
    
    const duration = typeof body.watchDuration === "number" ? Math.max(0, body.watchDuration) : 0;
    
    try {
        await database.update(shorts)
            .set({ 
    ...
    try {
        ...
        const result = await database
            .select({
                ...
                creator: users.username, creatorDisplayName: users.displayName, creatorAvatarUrl: users.avatarUrl,
                views: shorts.views, likesCount: shorts.likesCount, watchDuration: shorts.watchDuration,
                videoLength: shorts.videoLength, isModerated: shorts.isModerated, createdAt: shorts.createdAt
            })
            .from(shorts)
            .innerJoin(users, eq(shorts.userId, users.userId))
            .where(conditions)
...
shortsRoute.patch("/:id/moderate", requireAuth, async (c) => {
    ...
    const shortId = c.req.param("id");
    let body;
    try { body = await c.req.json(); } catch { return c.json({ success: false, code: "INVALID_JSON" }, 400); }

    if (typeof body.isModerated !== "boolean") return c.json({ success: false, code: "INVALID" }, 400);
...
// --- TEMPORARY MIGRATION ROUTE: BACKFILL DURATIONS ---
shortsRoute.post("/admin/backfill-durations", requireAuth, async (c) => {
        ...
        try {
            ...
            for (const video of videosToFix) {
                try {
                    ...
                    const filename = urlParts[urlParts.length - 1]; // e.g., video-id.mp4
                    const tempFilePath = path.join(os.tmpdir(), filename);
    ...
    return c.json({ success: true, message: "Backfill started with Temp Files. Check pod logs." });
});