...
quiz.post(
	...
	async (c) => {
		...
		// Validate name length and emptiness
...
// --- DELETE QUIZ ---
quiz.delete("/:quizId", requirePerm("quiz:delete"), async (c) => {
	const workspaceId = c.req.param("workspaceId");
	const teamId = c.req.param("teamId");
	const quizId = c.req.param("quizId");
...
quiz.get("/:quizId/collaborators", requirePerm("quiz:read"), async (c) => {
	const quizId = c.req.param("quizId");
	try {
...
quiz.post(
	...
	async (c) => {
		...
		try {
			const [newCollab] = await database
			...
			return c.json({ success: true, code: "COLLABORATOR_ADDED", collaborator: newCollab });
		} catch (error) {
			console.error("[Quiz API] Failed to add collaborator:", error);
			return c.json({ success: false, code: "INTERNAL_SERVER_ERROR" }, 500);
		}
	}
);

// --- STOP COLLABORATING (Self-Removal - Must be placed before /:userId) ---
...
quiz.delete("/:quizId/collaborators/:userId", requirePerm("quiz:edit"), async (c) => {
	const quizId = c.req.param("quizId");
